...
def load_file_data(logfile : Path, keypath : PurePath, is_date=False) -> List[Any]:
    ...
    with open(logfile) as f:
        ...
        while linejson := f.readline():
            ...
            if len(linejson) <= 3:
                ...
                # If is_date, parse into datetime
                if is_date:
                    linedate = parser.parse(linedata)
                    linedata = linedate
...
def run(logfile):
    ...
    x_data = load_file_data(logfile, x_axis_key,is_date=True)
